...
PROGRESS.prototype._render = function(to){
	...
	if(!_this._visible) {
		...
		if(_this.title != "") process.stdout.write("\n  " + cook(_this.title).addStyles(_this.color,"bold") + "\n");
		culinary.cursorTo(12).write(repeat(cook(" ").addStyles(_this.bg), width)).cursorTo(12);
	...
	culinary.write(cook(repeat(" ", length)).addStyles(_this.fg));
