...
PROGRESS.prototype._render = function(to){
	...
	if(!_this._visible) {
		...
		if(_this.title != "") process.stdout.write("\n  " + chalk[_this.color](_this.title) + "\n");
		process.stdout.cursorTo(12);
		process.stdout.write("" + repeat(chalk[_this.bg](" "), width));
		process.stdout.cursorTo(12);
		_this._visible = true;
	}else{
		process.stdout.write("\033[s"); // Save cursor position
		process.stdout.write("\033[1B"); // Move one line up
	...
	process.stdout.write(chalk[_this.fg](repeat(" ", length)));
